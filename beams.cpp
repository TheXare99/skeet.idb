...
void beams::listener(IGameEvent* game_event)
{
	...
	if (player != g_pLocalPlayer || !cfg.visuals.effects.bullet_tracers)
		return;
	...
	impact_info_t impact(g_pGlobals->curtime, player->get_eye_pos(), pos, player, cfg.visuals.effects.bullet_tracers_color);
...
{
	if (!cfg.visuals.effects.bullet_tracers)
		return;
