...
bool C_BasePlayer::is_point_visible(Vector endpos)
{
	trace_t tr;
	Ray_t ray;
	CTraceFilter filter;
	filter.pSkip = this;
	ray.Init(g_pLocalPlayer->get_eye_pos(), endpos);
	g_pTrace->TraceRay(ray, MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
	return tr.fraction > 0.97f;
}
bool C_BasePlayer::get_alive() 
{
	return this->get_lifestate() == LifeState::ALIVE; 
}
Vector C_BasePlayer::get_eye_pos()
{
	Vector pos;
	util::get_vfunc<void( __thiscall* )( void*, Vector& )>( this, 284 )( this, pos);
	pos.z -= get_view_offset().z - floor(get_view_offset().z);
	return pos;
}
bool C_BasePlayer::is_enemy() {
	__(mp_teammates_are_enemies_s, "mp_teammates_are_enemies");
	static ConVar* mp_teammates_are_enemies = g_pCVar->FindVar(mp_teammates_are_enemies_s);
	if (g_pLocalPlayer->get_team() != this->get_team())
		return true;

	else if (mp_teammates_are_enemies->get_int())
		return true;
...
void C_CSPlayer::set_anim_layers( std::array<C_AnimationLayer, 13>& layers )
{
