...
void autostrafer::strafe()
{
	...
	if (!cfg.misc.miscellaneous.air_strafe)
		return;
	...
	prediction::get().strafe_angles.y = math::get().normalize_float(prediction::get().strafe_angles.y + wish_dir);

	// cancel out any forwardmove values.
	g_cmd->forwardmove = 0.f;

	// get our viewangle change.
	delta = math::get().normalize_float(prediction::get().strafe_angles.y - old_yaw);

	// convert to absolute change.
	abs_delta = abs(delta);

	// save old yaw for next call.
	old_yaw = prediction::get().strafe_angles.y;
	...
	if (abs_delta <= ideal || abs_delta >= 30.f) {
		...
		velocity_delta = math::get().normalize_float(prediction::get().strafe_angles.y - velocity_angle.y);
		...
		if (velocity_delta <= correct || speed <= 15.f) {
			...
			if (-correct <= velocity_delta || speed <= 15.f) {
				prediction::get().strafe_angles.y += (ideal * switch_value);
				g_cmd->sidemove = cl_sidespeed->get_float() * switch_value;
			}

			else {
				prediction::get().strafe_angles.y = velocity_angle.y - correct;
				g_cmd->sidemove = cl_sidespeed->get_float();
			}
		}
