	...
	for (auto i = 1; i < g_pGlobals->maxClients; i++)
	{
		...
		if (player->IsDormant())
		{
			if (players[i].alpha < 160.f)
			{
				...
				players[i].alpha = clamp(players[i].alpha, 0.f, 160.f);
			}
			else
			{
...
{
	if (!cfg.visuals.player_esp.bounding_box)
		return;

	const auto color = Color(cfg.visuals.player_esp.bounding_box_color, player.alpha);
...
void visuals::player_info(const visual_data_t& player)
{
	...
	const auto info_left = [&]()
	{
		...
		if (cfg.visuals.player_esp.health_bar)
		{
	...
	const auto info_top = [&](const visual_data_t& player)
	{
		auto elements = 0;
		...
		if (cfg.visuals.player_esp.name)
		{
			auto color = Color(cfg.visuals.player_esp.name_color, player.alpha);
...
void visuals::bar_health(const visual_data_t& player, const int elements) const
{
	...
	// цвета для хп баров спиздил с супримаси.
	const int red = min((510 * (100 - health)) / 100, 255);
	const int green = min((510 * health) / 100, 255);
...
{
	if (!cfg.visuals.player_esp.out_of_fov_arrow)
		return;
	...
	const auto dist = 0.32f * dimensions.Height + height;
	...
	const auto col = Color(cfg.visuals.player_esp.out_of_fov_arrow_color, alpha);
...
void visuals::queue_world_data()
{
	...
	for (auto i = 1; i < g_pEntitiyList->GetHighestEntityIndex(); i++)
	{
		...
		{
			const auto cl_dist = clamp(dist - 300.f, 0.f, 510.f);
		...
		if (is_weapon(id))
		{
			...
			if (weapon)
				data.name = clean_item_name(weapon->GetClientClass()->m_pNetworkName);
...
void visuals::queue_player_data()
{
	...
	for (auto& player : players)
	{
		...
		if (weapon)
		{
			player.weapon_name = clean_item_name(weapon->GetClientClass()->m_pNetworkName);
		}
...
void visuals::defuse_bar(const world_data_t& ent) const
{
	...
	if (!was_defusing && ent.defusing)
	{
		...
		if (defuser)
		{
			if (defuser->has_defuser())
				defusetime = g_pGlobals->curtime + 5.f;
			else
				defusetime = g_pGlobals->curtime + 10.f;
	...
	auto multiplier = (defusetime - g_pGlobals->curtime) / 10.f;
	multiplier = clamp(multiplier, 0.f, 1.f);

	const auto red = clamp(static_cast<int>((1 - multiplier) * 160) + 160, 0, 255);
	const auto green = clamp(static_cast<int>(multiplier * 255), 0, 255);
...
void visuals::ammo_bar(const world_data_t& ent) const
{
	...
	render::get().text(Vector2D(ent.pos.x, ent.pos.y + 20), tfm::format( "%d / %d", ent.clip, ent.maxclip ), D3DCOLOR_ARGB(static_cast<int>(ent.alpha), 255, 255, 255), fonts::esp, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);
}
...
void visuals::draw_scope()
{
	...
	if ((observer && observer->get_scoped() || g_pLocalPlayer->get_scoped()) && cfg.visuals.effects.remove_scope_overlay)
	{
		const auto size = render::get().get_dimensions();
