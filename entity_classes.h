...
public:
	VFUNC(17, get_pred_desc_map(), datamap_t* (__thiscall*)(void*))()
		...
		NETVAR(get_view_offset, Vector, "CBaseEntity->m_vecViewOffset_0")
		...
		DATAMAPVAR(get_surface_friction, int, "m_surfaceFriction")
...
public:
	...
	bool get_alive();
	bool is_point_visible(Vector endpos);
	Vector get_eye_pos();
	bool is_enemy();
...
public:
	...
	OFFSET( get_context, CIKContext*, 0x2670 )
	OFFSET( get_occlusion_flags, int, 0xA28 )
	APOFFSET( get_anim_layers, C_AnimationLayer, 13, 0x2990)
	OFFSET( get_anim_layer_count, int, 0x298C )
	OFFSET( get_anim_state, CCSGOPlayerAnimState*, 0x3914)
	...
	void set_anim_layers( std::array<C_AnimationLayer, 13>& layers );
